"""
Schema validator för klient-sidan (uploader source-type-mappings).

Duplikat av server/utils/schema_validator.py — princip 1 (inga shared
Python-moduler mellan klient och server). Client bundle får inte
innehålla server-kod.
"""
...
class SchemaValidator:
    ...
    def validate_node(self, node_data: Dict[str, Any], strict: bool = False) -> Tuple[bool, str]:
        """Validera nod mot schemat.

        Required-, enum-, och typ-checkar körs alltid (baseline — ground zero
        för data som får ligga i index). `strict=True` aktiverar extra opt-in-
        checkar avsedda för testsviter och L3-invariant-rapport:

        - **Unknown-key-check:** varje nyckel i `node_data` måste vara
          deklarerad i schemat (base_properties eller typ-specifika
          properties). Fångar drift där koden uppfinner egna nycklar.
        - **Regex-check:** properties med `regex`-fält i schemat (t.ex.
          `Person.email`) valideras mot mönstret.

        Produktionskod (ingestion-writes) anropar normalt utan strict=True;
        testsviten och cross-tenant invariant-rapporten använder
        strict=True.
        """
        ...
        # --- Strict-mode-extras (opt-in) ---
        if strict:
            # Unknown-key-check: tillåtna = "type" (nodtyp-indikator) + schema-properties
            allowed_keys = {"type"} | set(properties_def.keys())
            unknown = set(node_data.keys()) - allowed_keys
            if unknown:
                return False, (
                    f"Unknown properties for node type '{node_type}' "
                    f"(not in schema): {sorted(unknown)}"
                )

            # Regex-check
            for prop_name, prop_def in properties_def.items():
                pattern = prop_def.get("regex")
                if not pattern:
                    continue
                value = node_data.get(prop_name)
                if value is None or not isinstance(value, str):
                    continue  # typ-kollen ovan har redan hanterat icke-string
                if not re.match(pattern, value):
                    return False, (
                        f"Regex mismatch for '{prop_name}': "
                        f"value {value!r} doesn't match {pattern!r}"
                    )
    ...
    def validate_edge(self, edge: Dict[str, Any], nodes_map: Dict[str, str], strict: bool = False) -> Tuple[bool, str]:
        """
        ...
        Required-, enum- och typ-checkar körs alltid. `strict=True` aktiverar
        samma opt-in-checkar som på validate_node:
        - Unknown-key-check (utöver "type", "source", "target" + schema-properties)
        - Regex-check för properties med `regex`-fält
        """
        ...
        for prop_name, prop_def in properties_def.items():
            ...
            if prop_name in edge:
                ...
                # Type check — använd 'type' för storage-validering (matchar
                # validate_node). `extraction_type` i schemat är en LLM-
                # prompt-hint som beskriver hur items extraheras (t.ex.
                # "string") — inte vad den lagrade listan innehåller.
                expected_type = prop_def.get("type")
                if expected_type:
        ...
        # --- Strict-mode-extras (opt-in) ---
        if strict:
            # Unknown-key-check: strukturella fält + schema-properties tillåtna
            allowed_keys = {"type", "source", "target"} | set(properties_def.keys())
            unknown = set(edge.keys()) - allowed_keys
            if unknown:
                return False, (
                    f"Edge '{rel_type}': unknown properties "
                    f"(not in schema): {sorted(unknown)}"
                )
            ...
            for prop_name, prop_def in properties_def.items():
                ...
                if not re.match(pattern, value):
                    return False, (
                        f"Edge '{rel_type}': regex mismatch for '{prop_name}': "
                        f"value {value!r} doesn't match {pattern!r}"
                    )
    ...
    def get_document_node_type(self) -> str:
        """Returnerar den systemiska Source-nodtypen.

        `Source` är ett reserverat namn — varje schema MÅSTE deklarera en
        `Source`-nodtyp med `primary_key_strategy='UUID'`. Detta är
        kontraktet mellan motorn och domän-schemat (architecture_principles
        princip 10). Tidigare implementation sökte dynamiskt efter UUID-PK-
        noden och skapade falsk agnostik — Source-namnet är systemgräns,
        inte domänval.

        Valideringen att `Source` faktiskt finns och har rätt PK-strategi
        sker i `tools/tests/contract/test_schema_validator.py` (gate vid
        test-körning, inte HARDFAIL i `__init__`).
        """
        return "Source"
