...
Auth (#234 plan B):
    Access-tokens läses per-request från menubar-brokerns cache-fil
    (~/Library/Caches/MyMemory/access_token). Ingen PAT längre.
    Saknas filen = soft-fail, status kvar som 'pending', ingen HARDFAIL.

Environment variables:
    MYMEMORY_CONFIG               Sökväg till my_mem_config.yaml
    MYMEMORY_UPLOAD_URL           Override för cloud.api_url
    MYMEMORY_ACCESS_TOKEN         Override (dev/test) för broker-filen
...
def _get_uploader_config() -> dict:
    ...
    Access-tokens läses inte här — de hämtas per request från
    client.utils.broker_auth.get_access_token() (menubar-brokerns cache-fil,
    #234). Uploadern startar även utan token; requests vilar i 'pending'
    tills brokern skrivit en färsk fil.
    """
    cfg = get_config()
    cloud = cfg.get('cloud', {})
...
class AuthMissingError(Exception):
    """Access-token saknas i broker-cachen. Soft-fail (vänta på menubar)."""


class AuthRejectedError(Exception):
    """Access-token avvisades (401) även efter re-read.

    Brokern hinner inte med, eller refresh-token är död. Soft-fail i uploadern —
    status kvar 'pending', ingen retry_count-påverkan (det är inte asset-fel).
    """
...
def _post_once(url: str, token: str, body: bytes, verify_tls: bool) -> tuple:
    """Enskild POST-försök. Returnerar (status_code, body_dict).

    Raises AuthRejectedError vid 401, UploadError vid 5xx/nätverk, bubblar
    2xx/4xx-svar till caller.
    """
    ...
    try:
        ...
        if e.code == 401:
            raise AuthRejectedError(str(err_body)) from e
        if e.code >= 500:
            raise UploadError(
...
def post_payload(api_url: str, payload: dict, verify_tls: bool = True) -> tuple:
    """POST /api/v1/ingest med färsk access-token från brokern.

    Returnerar (status_code, response_body_dict).

    Auth-kedja (#234):
      1. Läs access-token från broker-filen
      2. POST med token
      3. Om 401: re-read filen (brokern kan ha roterat precis) och försök
         en gång till
      4. Fortsatt 401 → AuthRejectedError (soft-fail i caller)
         Ingen token → AuthMissingError (dito)
    """
    ...
    try:
        return _post_once(url, token, body, verify_tls)
    except AuthRejectedError:
        # Läs om filen — brokern kan ha skrivit en ny token efter första POST:en
        fresh = reload_access_token()
        if fresh and fresh != token:
            LOGGER.info("Access-token roterad under request — retrying once")
            return _post_once(url, fresh, body, verify_tls)
        # Samma token eller saknas — brokern hinner inte med
        raise
...
def process_asset(state: UploaderState, cfg: dict, row: dict) -> str:
    ...
    try:
        ...
        # Menubar-brokern har inte skrivit access-token än (användaren inte
        # inloggad eller menubar just startat). Soft-fail: vila 60s, räkna
        # inte upp retry_count (det är inte asset-fel).
        LOGGER.info(f"{filename}: väntar på credentials (ingen access-token)")
        state.update_status(
            uuid_str, 'pending',
        ...
        # Token avvisades även efter re-read. Brokern rullar refresh var
        # 10 min — vänta 60s och försök igen. retry_count orört.
        LOGGER.warning(f"{filename}: access-token avvisad, väntar på ny: {e}")
        state.update_status(
            uuid_str, 'pending',
            next_retry_at=datetime.fromtimestamp(
                datetime.now(timezone.utc).timestamp() + 60,
...
def _run_sse_consumer(state: 'UploaderState', cfg: dict) -> None:
    ...
    Auth: access-token läses om vid varje reconnect-försök från broker-
    filen (#234). Om token saknas väntar vi i backoff-loopen tills menubar
    skrivit den. 401 under en pågående stream → close + reconnect med ny
    token nästa varv.
    ...
    while _running:
        token = get_access_token()
        if not token:
            LOGGER.debug("SSE: ingen access-token ännu — väntar på broker")
            for _ in range(5):
                if not _running:
                    break
                time.sleep(1)
        ...
        try:
            ...
            if e.code == 401:
                # Token har rollat eller blivit revokerad — brokern skriver
                # en ny inom 10 min. Kort backoff så vi snabbt plockar upp.
                LOGGER.info(f"SSE: 401 — väntar på ny access-token ({backoff}s)")
            else:
