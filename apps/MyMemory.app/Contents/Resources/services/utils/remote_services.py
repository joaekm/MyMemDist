...
Konfigureras via klient-config (cloud.api_url, cloud.verify_tls)
och Keychain-PAT (samma som uploader_daemon).
"""
...
def _get_pat() -> str:
    """Hämta PAT från macOS Keychain (samma källa som uploader_daemon)."""
    try:
        result = subprocess.run(
            ["security", "find-generic-password",
             "-s", "MyMemory", "-a", "uploader_pat", "-w"],
            capture_output=True, text=True, check=True,
        )
        return result.stdout.strip()
    except subprocess.CalledProcessError as exc:
        raise RuntimeError(
            "HARDFAIL: Ingen PAT i Keychain (MyMemory/uploader_pat). "
            "Generera på servern (/pat) och kör: "
            "security add-generic-password -U -s MyMemory -a uploader_pat -w '<TOKEN>'"
        ) from exc
...
@dataclass
class LLMResponse:
    """Kompatibel med services.utils.llm_service.LLMResponse."""
    text: str
    success: bool
